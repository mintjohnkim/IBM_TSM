...
import torch.hub
from epic_models.tsm import TSM
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #class_counts = 157 #(125,352)
    #segment_count = args.groups
    #base_model = 'resnet50'
    ...
    # #model.fc_verb.bias.requires_grad = True
    # def weights_init(m):
    #     import random
    #     try:
    #         #torch.nn.init.xavier_normal_(m.weight.data, gain=0.01)
    #         torch.nn.init.constant(m.weight, random.uniform(-0.01,0.01))
    #         torch.nn.init.constant(m.bias, random.uniform(-0.01,0.01))
    #     except:
    #         pass
    ...
    if args.pretrained is not None:
        ...
        ################
        #print("!!!!!!!!TRANSFER")
        # pretrained_output_num = 38
        # target_output_num = 138
        # model.fc = nn.Linear(512,pretrained_output_num)
        ################
        ...
        for k, v in state_dict.items():
            ...
            if not 'fc.weight' in name and not 'fc.bias' in name:
                print('--added')
        ...
        # model.fc = nn.Linear(512,target_output_num)
        ...
        print("!!!!!!!!!!!!")
        print('post', post_params[1])

        #print("!!!!!!!!TRANSFER")
        #model.fc = nn.Linear(1024,119)
        ################
    ...
    if args.distributed:
        ...
        # assign rank to 0
        #device = torch.device("cuda:5,6,7")
        #torch.cuda.set_device("cuda:5,6,7")
        #model = torch.nn.DataParallel(model, device_ids = [5,6,7]).cuda()
    ...
    #train_criterion.requires_grad = True
    # Data loading code
    if args.listfilepath == '':
        val_list = os.path.join(args.datadir, val_list_name)
    else:
        val_list = os.path.join(args.listfilepath, val_list_name)
    ...
    if args.listfilepath == '':    
        ...
        train_list = os.path.join(args.listfilepath, train_list_name)
    ...
    if args.evaluate:
        mAP_softmax_return_flag=False
        val_top1, val_top5, val_losses, val_speed, softmax_iter = validate(val_loader, model, val_criterion,
                                                             gpu_id=args.gpu,
                                                             mAP_softmax_return=mAP_softmax_return_flag)
        if args.rank == 0:
            logfile = open(os.path.join(log_folder, 'evaluate_log.log'), 'a')
            print(
                'Val@{}: \tLoss: {:4.4f}\tTop@1: {:.4f}\tTop@5: {:.4f}\tSpeed: {:.2f} ms/batch'.format(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            ########################
            if is_best and mAP_softmax_return_flag:
                #Save softmax_iter list to txt file
                with open(log_folder + '/charades_softmax_output.txt', 'w') as f:
                    for i in softmax_iter:
                        f.write(' '.join(i) + "\n")
